...
(function() {
	...
	function getSearchURL(baseUrl, data) {
		const artist = data.credits.find(([v]) => v.includes("歌") || v.includes("アーティスト"));
		...
		document.addEventListener("turbo:load", async () => {
			...
			for (const section of sections) {
				...
				for (const row of rows) {
					...
					if (rowMatched[1].includes("歌") || rowMatched[1].includes("アーティスト")) artists.push([rowMatched[1], rowMatched[2]]);
